class AudioProcessor {
    ...
    optimizeChunk(audioBuffer) {
        ...
        if (audioBuffer.length < CHUNK_CONSTRAINTS.MIN_SIZE) {
            console.log("⚠️ Chunk too small, padding to minimum size");
        ...
        if (audioBuffer.length > CHUNK_CONSTRAINTS.MAX_SIZE) {
            console.log("⚠️ Chunk too large, truncating to maximum size");
        ...
        if (remainder !== 0) {
            console.log("⚠️ Adjusting chunk to be multiple of 320 bytes");
    ...
    processOpenAIResponse(base64AudioChunk) {
        console.log("\n🔄 Processing OpenAI Audio Response");
